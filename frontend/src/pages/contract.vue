<template>
  <div class="page">
      ...
      <div class="black-layer">
        <div class="red-layer">
          <div class="green-layer">
            ...
            <p class="submission-date">DATE OF SUBMISSION: {{ currentDate }}</p>
    ...
    <!-- Display Organization Responsibilities -->
    <div class="form-section">
      <h5>PART II: COMMITMENTS AND RESPONSIBILITIES OF THE ORGANIZATION</h5>
      <div v-if="orgResponsibilities.length === 0">No organization responsibilities found.</div>
      <div v-else>
        <div v-for="(responsibility, index) in orgResponsibilities" :key="'orgResp' + index" class="objective-item">
          <span class="objective-index">{{ index + 1 }}</span>
          <textarea v-model="orgResponsibilities[index].responsibility" :placeholder="'Organization Responsibility ' + (index + 1)"></textarea>
        </div>
      </div>
    </div>

    <!-- Display Government Obligations -->
    <div class="form-section">
      <h5>PART III: COMMITMENT AND OBLIGATIONS OF THE GOVERNMENT</h5>
      <div v-if="govtObligations.length === 0">No government obligations found.</div>
      <div v-else>
        <div v-for="(obligation, index) in govtObligations" :key="'govtOb' + index" class="objective-item">
          <span class="objective-index">{{ index + 1 }}</span>
          <textarea v-model="govtObligations[index].govt_obligation" :placeholder="'Government Obligation ' + (index + 1)"></textarea>
        </div>
      </div>
    </div>

     <!-- Display Thematic Areas -->
     <h5>PART IV: THEMATIC AREAS, REFORMS AND JUSTIFICATIONS</h5>
    <div v-if="thematicAreas.length > 0">
      <div v-for="(thematicArea, index) in thematicAreas" :key="'thematicArea' + index">
        <h6><label class="form-label" for="thematicArea">THEMATIC AREA:</label> {{ String.fromCharCode(65 + index) }}. <br>{{ thematicArea.thematic_area }}</h6>
  <!-- Display Focus Areas for each Thematic Area -->
  <div class="form-section">
      <label class="form-label" for="focusAreas"><strong>FOCUS AREAS:</strong></label>
      <div class="textarea-container">
        <template v-for="(focusArea, j) in thematicArea.focus_areas" :key="focusArea.id">
          <div class="focus-area-item">
            <span class="objective-index focus-area-number">{{ j + 1 }}</span>
            <!-- Display the focus area details -->
            <span>{{ focusArea.focus_area }}</span>
          </div>
        </template>
      </div>
    </div>
