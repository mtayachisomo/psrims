<template>
  <div>
    <!-- Quarter Selection -->
    <div>
      <h5>QUARTER AGAINST ANNUAL LEVEL OF PERFORMANCE ANALYSIS</h5>
      ...
      <table class="acr-table">
        ...
        <tbody>
          <tr v-for="(row, index) in annualACRTableData" :key="index">
            ...
            <td>{{ row.annual_average_acr }}</td> <!-- Display annual_average_acr here -->
...
const fetchReformAreaACRData = async (outputReportingPeriodId, reformAreaId) => {
  try {
    const response = await api.get(`/annual-reformarea-output-acr/output-reporting-period/${outputReportingPeriodId}/reform-area/${reformAreaId}/`);
    ...
    if (data.length === 0) {
      ...
      while (previousQuarterId !== null) {
        const previousResponse = await api.get(`/annual-reformarea-output-acr/output-reporting-period/${previousQuarterId}/reform-area/${reformAreaId}/`);
        ...
        if (previousData.length > 0) {
          ...
          return previousData.map(item => ({
            ...
            annual_output_acrs: item.annual_output_acrs || [],
          }));
        }
    ...
    options: {
      ...
      onClick: (event, elements) => {
        if (elements.length > 0) {
          ...
          if (reformArea && reformArea.annual_output_acrs) {
            // Update the displayed data based on the clicked bar
            updateAnnualOutputACRs(reformArea.annual_output_acrs);
          }
        }
      },
    },
  ...
  try {
    ...
    const promises = annualOutputAcrs.map(async (acr) => {
      ...
      return {
        ...
        annual_average_acr: acr.annual_average_acr, // Add annual_average_acr here
